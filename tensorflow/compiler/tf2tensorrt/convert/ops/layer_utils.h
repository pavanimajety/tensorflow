...
class TRTNetworkBuilder {
 ...
 public:
  ...
  StatusOr<nvinfer1::ITensor*> ModifyDimsIfNegative(
    ...
    VLOG(2) << "llimit" << DebugString(llimit);
    VLOG(2) << "rlimit" << DebugString(rlimit);
    VLOG(2) << "indices tensor" << DebugString(indicesT);
    auto signs = this->ClampDims(indicesT, llimit, rlimit);
